...
SEED = 42
LR = 1e-4
MODEL_SAVE = 'best_flood_unet.pth'
...
def gather_triplets(root):
    ...
    for scene in root.iterdir():
        tiles=scene/'tiles'
        vh_d, vv_d, m_d = tiles/'vh', tiles/'vv', tiles/'flood_label'
...
def main():
    ...
    plot_metrics(train_loss_hist, val_loss_hist, 'loss', 'Training & Validation Loss', 'flood_label_training_metrics_pytorch.png')
    plot_metrics(train_acc_hist,  val_acc_hist,  'accuracy', 'Training & Validation Accuracy', 'flood_label_training_metrics_pytorch.png')
    plot_metrics(train_iou_hist,  val_iou_hist,  'IoU', 'Training & Validation IoU', 'flood_label_training_metrics_pytorch.png')

    # Save sample predictions
    show_sample_preds(model, val_ds, 'flood_label_sample_predictions_pytorch.png', n=SAMPLE_COUNT)
