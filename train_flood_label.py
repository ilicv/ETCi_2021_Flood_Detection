...
vh_folder = 'vh'
vv_folder = 'vv'
mask_folder = 'flood_label'

# === Data gathering ===

def gather_tile_paths(root):
    """
    Walk dataset_root, find matching VH, VV and mask tiles.
    Returns lists of (vh_path, vv_path, mask_path).
    """
    triplets = []
    for scene_dir in root.iterdir():
        ...
        if vh_dir.is_dir() and vv_dir.is_dir() and mask_dir.is_dir():
...
def load_batch(triplets):
    X = np.zeros((len(triplets), IMG_SIZE, IMG_SIZE, 2), dtype=np.float32)
    y = np.zeros((len(triplets), IMG_SIZE, IMG_SIZE, 1), dtype=np.float32)
    for i, (vh_path, vv_path, mask_path) in enumerate(triplets):
...
# === Training setup ===
print("Building U-Net model for flood segmentation...")
model = unet()
model.compile(
    optimizer=Adam(1e-4),
...
def plot_history(hist):
    ...
    plt.tight_layout()
    plt.savefig('flood_label_training_metrics.png')
    plt.show()
...
def show_preds(model, triplets, n=6):
    ...
    plt.tight_layout()
    plt.savefig('flood_label_sample_predictions.png')
    plt.show()
